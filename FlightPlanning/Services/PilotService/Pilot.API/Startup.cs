...
using Pilot.API.EventBusConsumers;
using MassTransit;
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddMassTransit(config => {
                config.AddConsumer<PilotChecoutConsumer>();
                config.UsingRabbitMq((ctx, cfg) => {
                    cfg.Host(Configuration["EventBusSettings:HostAddress"]);
                    cfg.UseHealthCheck(ctx);
                    cfg.ReceiveEndpoint(EventBusConstants.PilotCheckoutQueue, config => {
                        config.ConfigureConsumer<PilotChecoutConsumer>(ctx);
                    });
                });
            });
            ...
            services.AddScoped<PilotChecoutConsumer>();
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "Pilot.API", Version = "v1" });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Pilot.API v1"));
            }
